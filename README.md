...
Многопоточная реализация градиентного бустинга на oblivious decision trees.
...
Для экспериментов задействованы одинаковые наборы гиперпараметров (тк мы реализуем лишь незначительный набор регуляризаций и для сокращения возможного перебора гиперпараметров фреймворков-конкурентов утилиты).
Эксперименты состоят из перебора гиперпараметров и последующей оценки качества с помощью кроссвалидации на 70% данных, и обучения на этих 70% и оценке лучшей полученной в ходе кроссвалидации модели на 30% holdout выборке.
